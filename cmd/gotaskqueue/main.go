...
func main() {
	...
	cfg := config.Load()
	logger.Info("configuration loaded", "http_addr", cfg.HTTP.Addr, "redis_addr", cfg.Redis.Addr, "redis_stream_name", cfg.Redis.StreamName, "redis_consumer_group", cfg.Redis.ConsumerGroup, "redis_consumer_name", cfg.Redis.ConsumerName, "postgres_host", cfg.Postgres.Host, "postgres_port", cfg.Postgres.Port, "postgres_database", cfg.Postgres.Database, "prometheus_url", cfg.Prometheus.URL, "scheduler_interval_seconds", cfg.Scheduler.IntervalSeconds, "scheduler_batch_size", cfg.Scheduler.BatchSize)
